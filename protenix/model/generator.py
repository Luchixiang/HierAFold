...
def sample_consistency(
        consistency_net: Callable,
        inference_scheduler: InferenceNoiseScheduler,
        ts: List[int],
        input_feature_dict: dict[str, Any],
        s_inputs: torch.Tensor,
        s_trunk: torch.Tensor,
    ...
    rho = inference_scheduler.rho
    t_max_rho = inference_scheduler.s_max ** (1 / rho)
    t_min_rho = inference_scheduler.s_min ** (1 / rho)
    x_l = noise_schedule[0] * torch.randn(
        size=(*batch_shape, N_sample, N_atom, 3), device=device, dtype=dtype
    )  # NOTE: set seed in distributed training

    for i in range(len(ts) - 1):
        # t = torch.tensor(inference_scheduler.sigma_data * (
        #             (t_max_rho + ts[i] / (N_step) * (t_min_rho - t_max_rho)) ** rho) ** rho, device=device)
        t = torch.tensor(inference_scheduler.sigma_data * (t_max_rho + ts[i] / N_step * (t_min_rho - t_max_rho)) ** rho,
                     device=device)
        t  = (
                t.reshape((1,) * (len(batch_shape) + 1))
                .expand(*batch_shape, N_sample)
                .to(dtype)
            )
        x_denoised = consistency_net(
            x_noisy=x_l,
            t_hat_noise_level=t,
            input_feature_dict=input_feature_dict,
            s_inputs=s_inputs,
        ...
        next_t = torch.tensor(inference_scheduler.sigma_data * (t_max_rho + ts[i+1] / N_step * (t_min_rho - t_max_rho)) ** rho,
                     device=device)
        next_t = torch.clip(next_t, inference_scheduler.s_min, inference_scheduler.s_max)
        if i == len(ts) - 2:
            x_l = x_denoised
        else:
            x_l = x_denoised + torch.randn(
            size=(*batch_shape, N_sample, N_atom, 3), device=device, dtype=dtype) * torch.sqrt(next_t**2 - inference_scheduler.s_min**2)
    return x_l
...
def sample_consistency_training(
        denoise_net: Callable,
        consitency_net: Callable,
        target_net: Callable,
    ...
    Returns:
        torch.Tensor: the denoised coordinates of x in inference stage
            [..., N_sample, N_atom, 3]
    """
    def henu_solver(denoise_net, x_l, c_tau_last, c_tau, step_scale_eta):
        x_denoised = denoise_net(
            x_noisy=x_l,
            t_hat_noise_level=c_tau_last,
            input_feature_dict=input_feature_dict,
            s_inputs=s_inputs,
            s_trunk=s_trunk,
            z_trunk=z_trunk,
        ...
        d = (x_l - x_denoised) / c_tau_last[
            ..., None, None
        ]
        dt = c_tau - c_tau_last
        samples = x_l + step_scale_eta * dt[..., None, None] * d
        x_denoised = denoise_net(
            x_noisy=samples,
            t_hat_noise_level=c_tau,
            input_feature_dict=input_feature_dict,
            s_inputs=s_inputs,
            s_trunk=s_trunk,
            z_trunk=z_trunk,
        ...
        next_d = (samples - x_denoised) / c_tau[
            ..., None, None
        ]
        dt = (c_tau - c_tau_last) / 2
        samples = x_l + step_scale_eta * (d + next_d) * dt[..., None, None]
        return samples
    ...
    # Add independent noise to each structure
    # sigma: independent noise-level [..., N_sample]
    random_indice = torch.randint(
        0, step_list.shape[0] - 1, size=(*batch_size_shape, N_sample), device=device
    )
    t = step_list[random_indice]
    t2 = step_list[random_indice + 1]
    # noise: [..., N_sample, N_atom, 3]
    noise = torch.randn_like(x_gt_augment, dtype=dtype) * t[..., None, None]
    # Get denoising outputs [..., N_sample, N_atom, 3]
    x_t = x_gt_augment + noise
    weights = 1 / torch.abs(t - t2)  # todo: update the weights
    dropout_state = torch.get_rng_state()
    distiller = consitency_net(
        x_noisy=x_t,
        t_hat_noise_level=t,
        input_feature_dict=input_feature_dict,
        s_inputs=s_inputs,
    ...
    # x_t2 = solver2(denoise_net, x_t, t, t2, step_scale_eta=1.0, gamma_min=1.0, gamma0=0.8,
    #                noise_scale_lambda=1.003).detach()
    x_t2 = henu_solver(denoise_net, x_t, t, t2, step_scale_eta=1.).detach()
    torch.set_rng_state(dropout_state)
    with torch.no_grad():
        distiller_target = consitency_net(
            x_noisy=x_t2,
            t_hat_noise_level=t2,
            input_feature_dict=input_feature_dict,
            s_inputs=s_inputs,
            s_trunk=s_trunk,
            z_trunk=z_trunk,
    ...
    c = 0.08
    distiller = distiller * coordinate_mask[..., None, :, None]
    distiller_target = distiller_target * coordinate_mask[..., None, :, None]
    diffs = torch.sqrt((distiller - distiller_target) ** 2 + c ** 2) - c
    diffs = torch.mean(diffs, dim=list(range(1, len(diffs.shape))))
    loss = diffs * weights
    return loss.mean()
