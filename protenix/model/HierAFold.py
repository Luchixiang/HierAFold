...
def _min_dist_chunked(
    query_coords: torch.Tensor,   # [N_q, 3]
    other_coords: torch.Tensor,   # [N_o, 3]
    chunk_size: int = 256,
) -> torch.Tensor:
    """
    Compute per-other-token minimum distance to any query token,
    without materialising the full [N_q, N_o] matrix.
    Returns: [N_o] tensor of minimum distances.
    """
    N_o = other_coords.shape[0]
    min_dists = torch.full((N_o,), float("inf"), device=other_coords.device)
    for start in range(0, query_coords.shape[0], chunk_size):
        q_chunk = query_coords[start : start + chunk_size]          # [C, 3]
        d_chunk = torch.cdist(q_chunk, other_coords)                # [C, N_o]
        min_dists = torch.minimum(min_dists, d_chunk.min(dim=0).values)
    return min_dists
...
class HierAFold(nn.Module):
    ...
    def select_context_tokens_optimized(
            self,
            query_chain_id: int,
        ...
        # ------------------------------------------------------------------ #
        # OPT-1: Vectorized per-token helper arrays (replace Python loop)     #
        # ------------------------------------------------------------------ #
        # Before: Python for-loop with N_token iterations
        # After:  3 vectorized index operations — ~100x faster for N>1000
        centre_atom_index = frame_atom_index[:, 1]  # [N_token] — no loop
        is_resolved_token = is_resolved[centre_atom_index]  # [N_token] bool
        is_ligand_token = is_ligand[centre_atom_index]  # [N_token] bool

        # ------------------------------------------------------------------ #
        # OPT-2: Pre-compute token centres once                               #
        # ------------------------------------------------------------------ #
        token_centers = pred_coordinates[centre_atom_index]  # [N_token, 3]

        # ------------------------------------------------------------------ #
        # OPT-3: Pre-compute PAE once in uint8 on CPU, slice to GPU on demand #
        # ------------------------------------------------------------------ #
        # Keeping pae_discrete on CPU avoids holding a large [N,N] GPU tensor
        # across the entire chain loop. Each slice is small and moved on demand.
        if pae is not None:
            # argmax over bins → uint8 (max 255 bins, saves 8x vs int64)
            pae_discrete_cpu = torch.argmax(pae, dim=-1).to(torch.uint8).cpu()
            del pae  # free raw logits immediately
        else:
            pae_discrete_cpu = None

        # ------------------------------------------------------------------ #
        # OPT-4: Pre-compute all chain masks at once (avoid repeated `where`) #
        # ------------------------------------------------------------------ #
        # Before: torch.where(asym_id == X) called inside every loop iteration
        # After:  build a dict of masks once — O(n_chains) total work
        all_polymer_chain_ids = torch.unique(asym_id[~is_ligand_token])
        other_chain_ids = all_polymer_chain_ids[
            all_polymer_chain_ids != query_chain_id
            ]

        # Pre-build token index lists for every chain we'll need
        chain_token_cache: dict = {}
        for cid in all_polymer_chain_ids.tolist():
            resolved_mask = (asym_id == cid) & (~is_ligand_token) & is_resolved_token
            chain_token_cache[cid] = {
                "resolved": torch.where(resolved_mask)[0],
                "unresolved": torch.where(
                    (asym_id == cid) & (~is_ligand_token) & (~is_resolved_token)
                )[0],
            }
        ...
        query_resolved_tokens = chain_token_cache[query_chain_id]["resolved"]
        query_center_coords = token_centers[query_resolved_tokens]  # [N_q, 3]

        # Use a boolean presence tensor instead of a list of tensors
        # This makes deduplication O(1) per token instead of O(N) per cat+unique
        N_total = N_token
        selected_mask = torch.zeros(N_total, dtype=torch.bool, device=device)

        selected_tokens_list: List[torch.Tensor] = [query_tokens]
        selected_mask[query_tokens] = True
        ...
        # ------------------------------------------------------------------ #
        # OPT-5: Pre-slice query PAE row once (reused for every other chain)  #
        # ------------------------------------------------------------------ #
        # Before: pae_discrete[query_resolved_tokens][:, other_resolved_tokens]
        #         — re-indexes query rows for EVERY other chain
        # After:  slice query rows once, then column-index per chain
        if pae_discrete_cpu is not None:
            # Move only the query rows to GPU once — [N_q, N_token]
            pae_query_rows = pae_discrete_cpu[
                query_resolved_tokens.cpu()
            ].to(device)  # [N_q, N_token]  (uint8, small)
        else:
            pae_query_rows = None
        ...
        for other_id in other_chain_ids.tolist():
            other_resolved_tokens = chain_token_cache[other_id]["resolved"]
            if len(other_resolved_tokens) == 0:
                continue

            # Small chains — keep in full, skip expensive computation
            if len(other_resolved_tokens) <= 40:
                priority_info[f"small_chain_{other_id}"] = (
                    1.0, global_token_indices[other_resolved_tokens], False
                )
                selected_tokens_list.append(other_resolved_tokens)
                subunit_counter += 1
                continue
            ...
            # ------------------------------------------------------------------ #
            # OPT-6: Chunked distance — never materialise full [N_q, N_o] matrix #
            # ------------------------------------------------------------------ #
            min_dist_per_other_token = _min_dist_chunked(
                query_center_coords, other_centers, chunk_size=128
            )  # [N_o]  — peak memory O(128 × N_o) instead of O(N_q × N_o)
            ...
            cutoff_idx = torch.searchsorted(sorted_min_dist, 40.0).item()
            ...
            # ------------------------------------------------------------------ #
            # OPT-7: PAE cross-slice — column index only (query rows pre-sliced) #
            # ------------------------------------------------------------------ #
            if pae_query_rows is not None:
                # [N_q, N_o] — only column indexing needed now
                pae_cross = pae_query_rows[:, other_resolved_tokens].float()
            else:
                pae_cross = None

            # ------------------------------------------------------------------ #
            # OPT-8: Compute both quantile thresholds in one sorted pass         #
            # ------------------------------------------------------------------ #
            # Before: torch.quantile(..., 0.2) then torch.quantile(..., 0.5)
            #         — two full sorts of the same data
            # After:  sort once, index at both quantile positions
            context_selected_tokens = distance_selected_tokens
            if pae_cross is not None:
                # Sort along query dim once → reuse for both thresholds
                pae_sorted, _ = torch.sort(pae_cross, dim=0)  # [N_q, N_o]
                q20_idx = max(0, int(0.2 * pae_cross.shape[0]) - 1)
                q50_idx = max(0, int(0.5 * pae_cross.shape[0]) - 1)
                pae_q20 = pae_sorted[q20_idx]  # [N_o]
                pae_q50 = pae_sorted[q50_idx]  # [N_o]

                pae_selected = other_resolved_tokens[pae_q20 <= 10]
                if pae_selected.shape[0] >= other_resolved_tokens.shape[0] * 0.7:
                    pae_selected = other_resolved_tokens[pae_q50 <= 3]
            ...
            # ------------------------------------------------------------------ #
            # Domain decomposition (unchanged logic, optimized PAE access)        #
            # ------------------------------------------------------------------ #
            replicate = 1
            subunits = None
            ...
            if gff3_dir is not None and pdb_id is not None and os.path.exists(gff3_dir):
                ...
                subunits = parse_gff3_domains(
                    gff3_path, sequence_length, replicate=replicate
                )

            if subunits is None or len(subunits) == replicate:
                if pae_discrete_cpu is not None:
                    # OPT-9: intra-chain PAE — fetch from CPU, avoid double GPU alloc
                    other_cpu = other_resolved_tokens.cpu()
                    pae_intra = pae_discrete_cpu[other_cpu][:, other_cpu].to(device)
                    if pae_intra.max() > 10:
                        domain_boundaries = get_domain_splits(
                            pae_intra.cpu().numpy(), 10
                        )
                        subunits = [
            ...
            # ------------------------------------------------------------------ #
            # OPT-10: Per-domain PAE — vectorized median via pre-sorted columns   #
            # ------------------------------------------------------------------ #
            if len(subunits) > 1 and pae_cross is not None:
                # Sort pae_cross columns once for all domains
                # Shape: [N_q, N_o] already computed above
                pae_col_sorted, _ = torch.sort(pae_cross, dim=0)  # [N_q, N_o]
                pae_row_q50_idx = max(0, int(0.5 * pae_cross.shape[0]) - 1)

                domain_tokens_to_keep: List[torch.Tensor] = []
                soft_candidates: List[tuple] = []

                for span in subunits:
                    domain_tokens = other_resolved_tokens[span[0]: span[1]]
                    domain_centers = token_centers[domain_tokens]

                    # OPT-11: Chunked domain distance (avoids [N_q, N_domain] alloc)
                    min_domain_dist = _min_dist_chunked(
                        query_center_coords, domain_centers, chunk_size=128
                    ).min().item()

                    # Reuse pre-sorted columns — just slice the domain columns
                    domain_col_slice = pae_col_sorted[:, span[0]: span[1]]  # [N_q, N_d]
                    # Median over query dim (already sorted) → [N_d]
                    per_token_median = domain_col_slice[pae_row_q50_idx]
                    # Median over domain tokens
                    per_token_sorted, _ = torch.sort(per_token_median)
                    domain_q50_idx = max(0, int(0.5 * per_token_sorted.shape[0]) - 1)
                    pae_domain_score = per_token_sorted[domain_q50_idx].float()
                ...
                if soft_candidates:
                    ...
                    _, nearest_tokens, _, _, nearest_priority = soft_candidates[0]
                ...
                context_selected_tokens = torch.cat(domain_tokens_to_keep) \
                    if domain_tokens_to_keep else context_selected_tokens

            else:
                # Single domain
                min_dist = min_dist_per_other_token.min().item()  # reuse already computed
                pae_score = (
                    float(pae_cross.quantile(0.5).item())
                    if pae_cross is not None else 0.0
                )
            ...
            selected_tokens_list.append(context_selected_tokens)

            # Unresolved tokens
            unresolved_tokens = chain_token_cache[other_id]["unresolved"]
            if len(unresolved_tokens) > 0:
                priority_info[f"chain_{other_id}_unresolved"] = (
                    1000.0, global_token_indices[unresolved_tokens], False
                )
                selected_tokens_list.append(unresolved_tokens)

            # Free per-chain GPU tensors immediately
            del pae_cross, other_centers, min_dist_per_other_token
            if 'pae_col_sorted' in dir():
                del pae_col_sorted
        ...
        if len(ligand_tokens) > 0:
            ...
            selected_tokens_list.append(ligand_tokens)

        # ------------------------------------------------------------------ #
        # OPT-12: Final dedup via boolean mask (O(N) vs O(N log N) unique)    #
        # ------------------------------------------------------------------ #
        # Before: torch.cat(...).unique() — sorts entire concatenated tensor
        # After:  boolean mask set, already O(1) per token during loop
        for t in selected_tokens_list[1:]:  # query already set
            selected_mask[t] = True

        selected_tokens_tensor = torch.where(selected_mask)[0]  # already sorted!
        # No need for explicit sort — torch.where returns indices in ascending order
    ...
    def create_subset_input_feature_dict(
        ...
        for key in per_token_keys:
            if key in input_feature_dict:
                subset_dict[key] = input_feature_dict[key][selected_tokens]
        ...
        for key in atom_keys:
            if key in input_feature_dict:
                subset_dict[key] = input_feature_dict[key][selected_atoms].cuda()
        ...
        for key in ["resolution"]:
            if key in input_feature_dict:
                subset_dict[key] = input_feature_dict[key]

        # MSA features
        if "msa" in input_feature_dict:
            subset_dict["msa"] = (
                input_feature_dict["msa"][:, selected_tokens.cpu()].cuda()
            )
            subset_dict["has_deletion"] = (
                input_feature_dict["has_deletion"][:, selected_tokens.cpu()].cuda()
            )
            subset_dict["deletion_value"] = (
                input_feature_dict["deletion_value"][:, selected_tokens.cpu()].cuda()
            )
        ...
        if "coordinate" in input_feature_dict:
            subset_dict["coordinate"] = (
                input_feature_dict["coordinate"][selected_atoms, :]
            )
        ...
        **Stage 2 — Per-chain full prediction.**
        ...
        for idx_i in range(n_chains):
            ...
            for idx_j in range(idx_i + 1, n_chains):
                ...
                sel_i, priority_i = self.select_context_tokens_optimized(
                ...
                sel_j, priority_j = self.select_context_tokens_optimized(
